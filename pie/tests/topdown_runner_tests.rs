...
use crate::common::{CheckErrorExt, CommonOutput, CommonTask, Pie, ReadStringFromFile, ToLowerCase, WriteStringToFile};
...
fn test_require_task(mut pie: Pie<CommonTask>, temp_dir: TempDir) {
  ...
  pie.run_in_session(|mut session| {
    ...
    let to_lowercase_task_dyn = CommonTask::ToLowerCase(ToLowerCase("HELLO WORLD!".to_string()));
