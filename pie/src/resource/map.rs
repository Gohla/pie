...
impl<K: MapKey> MapWriter<'_, K> {
  ...
  #[inline]
  pub fn insert(&mut self, value: K::Value) -> Option<K::Value> where
    K: Clone,
  {
  ...
  #[inline]
  pub fn entry(&mut self) -> Entry<K, K::Value> where
    K: Clone,
  {
...
impl PartialEq for MapKeyObjToObj { // Manual impl because derive is borked.
  #[inline]
