...
impl ResourceChecker<PathBuf> for ModifiedChecker {
  ...
  #[inline]
  fn is_inconsistent<RS: ResourceState<PathBuf>>(
    &self,
    path: &PathBuf,
    _state: &mut RS,
    stamp: &Self::Stamp,
  ...
  mod test {
    ...
    fn test_hash_checker() -> TestResult {
      ...
      let stamp = {
        ...
        assert_matches!(checker.is_inconsistent(&path, &mut state, &stamp)?, None);
      ...
      let stamp = {
        ...
        assert_matches!(checker.is_inconsistent(&path, &mut state, &new_stamp)?, None);
        // But not with (old) `stamp` that was made before modifying the file.
        assert_matches!(checker.is_inconsistent(&path, &mut state, &stamp)?, Some(s) if s == new_stamp);
