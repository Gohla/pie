...
mod test {
  ...
  fn test_dependencies() {
    ...
    let require_b2a = TaskDependency::from_typed(task_a, EqualsChecker, output_a).into_require();
  ...
  fn test_add_task_dependency_panics() {
    ...
    let mut store: Store = Store::default();
    let dependency = TaskDependency::from_typed(task, EqualsChecker, output).into_require();
